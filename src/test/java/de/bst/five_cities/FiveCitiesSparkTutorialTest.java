...
import static de.bst.five_cities.FiveCitiesSparkTutorialTest.Mode.*;
import static de.bst.five_cities.utils.SparkUtils.*;
...
import de.bst.five_cities.utils.test.AbstractTest;
import scala.Tuple2;
...
public class FiveCitiesSparkTutorialTest extends AbstractTest {
	...
	// !HIDE
	{
		if (geoObjectsRDDcache.containsKey(countryId))
			return geoObjectsRDDcache.get(countryId);

		Broadcast<Map<String, Integer>> columnMappingsBroadcast = getSparkContext()
				.broadcast(OpenGeoDB.getTSVColumnMapping(countryId));

		JavaRDD<GeoObject> geoRDD = getSparkContext() //
				.textFile(OpenGeoDB.getTSVFile(countryId).getPath()) //
				.filter(line -> !line.startsWith("#")) //
				.map(line -> new GeoObject(line, columnMappingsBroadcast)) //
				.filter(geo -> mode == ALL || geo.hasPosition()) //
				.cache();

		geoObjectsRDDcache.put(countryId, geoRDD);
		return geoRDD;
	}
	...
	Map<String, JavaRDD<GeoObject>> geoObjectsRDDcache = new HashMap<>();
	...
	Instant start;
	Instant end;
	long distanceCalculationsStart = 0;

	long distanceCalculationsEnd = 0;

	JavaRDD<?> rddProcessed;

	public void startRecordingStatistics(JavaRDD<?> rdd) {
		distanceCalculationsStart = GeoDistance.countDistancesCalculations.get();
		rddProcessed = rdd;
		start = Instant.now();
	}

	public void endRecordingStatistics() {
